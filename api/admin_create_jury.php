...
try {
  $stmt = $pdo->prepare("INSERT INTO users (name,email,password_hash,password_plain,role) VALUES (?,?,?,?,'jury')");
  $stmt->execute([$name,$email,$hash,$password]);
} catch (Throwable $e) {
  json_out(["ok"=>false,"error"=>"Email exists"], 409);
}
json_out(["ok"=>true, "users"=>api_users_for_admin($pdo)]);
