...
<div id="view-auth" class="view">
  ...
  <div style="display:flex;align-items:center;justify-content:center;min-height:100vh;padding:40px 20px;">
    <div class="auth-card fade-in">
      ...
      <p style="text-align:center;font-size:12px;color:var(--text-dim);margin-top:14px;opacity:.6;">تجريبي: admin@majaaz.iq / admin123</p>
    </div>
  </div>
</div>
...
document.getElementById('btn-add-project').addEventListener('click',function(){
  ...
  (async()=>{ try{ const res = await API.post('api/admin_create_project.php', {
    ...
    createdAt:Date.now()
  }); DB.projects = res.projects; toast('تم إضافة المشروع', 'success'); }catch(e){ toast('تعذر إضافة المشروع', 'error'); } })();
...
document.getElementById('btn-add-jury').addEventListener('click',function(){
  ...
  (async()=>{ try{ const res = await API.post('api/admin_create_jury.php', {id:genId(),email:email,password:pass,role:'jury',name:name}); DB.users = res.users; toast('تم إنشاء حساب المحكم', 'success'); }catch(e){ toast('تعذر إنشاء الحساب', 'error'); } })();
...
function doLogin(){
  ...
  var user=null;
  for(var i=0;i<DB.users.length;i++){ if(DB.users[i].email===email&&DB.users[i].password===pass){ user=DB.users[i]; break; } }
  if(!user){ document.getElementById('auth-error').classList.add('show'); return; }
  DB.currentUser=user;
  document.getElementById('auth-error').classList.remove('show');
  if(user.role==='admin'){ showView('admin'); renderAdminSection('overview'); }
  else {
    document.getElementById('jury-sidebar-name').textContent=user.name;
    document.getElementById('jury-avatar').textContent=user.name[0]||'J';
    showView('jury'); renderJuryProjects();
  }
}
function doLogout(){ DB.currentUser=null; document.getElementById('login-email').value=''; document.getElementById('login-pass').value=''; showView('public'); }
...
function juryTab(tab){
  ...
  document.querySelectorAll('#view-jury .dash-section').forEach(function(s){ s.classList.remove('active'); });
  document.getElementById('jury-'+tab).classList.add('active');
